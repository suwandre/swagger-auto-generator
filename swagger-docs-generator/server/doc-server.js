...
class DocServer {
  ...
  setupRoutes() {
    ...
    // Dynamic swagger UI for each service/file combination
    this.app.get('/docs/:service/:fileType', (req, res) => {
      const { service, fileType } = req.params;
      const swaggerPath = path.join(this.docsDir, service, `${fileType}.auto-swagger.json`);
      
      console.log(`📄 Loading: ${swaggerPath}`);
      
      if (!fs.existsSync(swaggerPath)) {
        console.log(`❌ File not found: ${swaggerPath}`);
        return res.status(404).send(`
          <h1>❌ Documentation Not Found</h1>
          <p>Documentation for <strong>${service}/${fileType}</strong> not found</p>
          <p>Expected path: ${swaggerPath}</p>
          <p><a href="/">← Back to index</a></p>
        `);
      }
      ...
      try {
        const swaggerDocument = JSON.parse(fs.readFileSync(swaggerPath, 'utf8'));
        console.log(`✅ Loaded swagger doc with ${Object.keys(swaggerDocument.paths || {}).length} paths`);
        
        // Custom Swagger UI HTML with embedded document
        const swaggerHtml = this.generateSwaggerHTML(swaggerDocument, service, fileType);
        res.send(swaggerHtml);
      } catch (error) {
        console.error(`❌ Error loading swagger file: ${error.message}`);
        res.status(500).send(`
          <h1>❌ Error Loading Documentation</h1>
          <p>Error parsing swagger file for <strong>${service}/${fileType}</strong></p>
  ...
  generateSwaggerHTML(swaggerDocument, service, fileType) {
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        ...
        <title>${service} - ${fileType} API Documentation</title>
